...
import ManageUserPermission from "../users/ManageUserPermission";

const ManageExporter = ({ onSupplierSelect }) => {
          ...
          renderCell: (params) => {
            return (
              <>
                ...
                <IconButton
                  onClick={() => navigate(`/form/expoters/${params.row.id}`)}
                >
                  <Edit color="primary" />
                </IconButton>
                <IconButton
          ...
          columns.push({
            ...
            renderCell: (params) => {
              return (
                <Button
                  onClick={() =>
                    showJSONData(params.row.addition_details, "Additional")
                  }
          ...
          columns.push({
            ...
            renderCell: (params) => {
              return (
                <Button
                  onClick={() =>
                    showJSONData(params.row.social_media_links, "Social Links")
                  }
                  startIcon={<ViewCompact />}
                  variant="contained"
                >
    ...
    <Box component={"div"} sx={{ width: "100%" }}>
      {!onSupplierSelect && (
        <Box display={"flex"} justifyContent={"space-between"}>
          <Breadcrumbs>
            ...
            <Typography
              variant="body2"
              onClick={() => navigate("/manage/expoters")}
            >
      ...
      <Grid container spacing={2}>
        <Grid
          item
          xs={12}
          sm={showImages || showAddReviews ? 8 : 12}
          lg={showImages || showAddReviews ? 9 : 12}
        >
          <Grid container spacing={2}>
                      ...
                      <Grid item key={index} xs={12} sm={6} md={4} lg={3}>
                        {field.option ? (
                          <Autocomplete
                            ...
                            value={
                              field.option.find(
                                (option) => option.id === watch(field.key)
                              ) || null
                            }
        ...
        {showAddReviews && (
          ...
          >
            <Box m={2} display={"flex"} justifyContent={"space-between"}>
              ...
              <IconButton onClick={() => setShowAddReviews(false)}>
                <Close />
              </IconButton>
            </Box>
            <Divider />
            <FormProvider {...methods}>
              <form onSubmit={methods.handleSubmit(onSubmitAddReview)}>
                ...
                <Autocomplete
                  ...
                  defaultValue={
                    userList.find(
                      (option) => option.id === watch("review_user_id")
                    ) || null
                  }
                  onChange={(event, newValue) => {
        ...
        <DialogContent>
          ...
          {jsonData.map((item, index) => (
            <React.Fragment key={index}>
              <Typography mt={2}>
                <Circle sx={{ fontSize: "12px", marginRight: "10px" }} />{" "}
                {item.key} - {item.value}
              </Typography>
              <Divider />
            </React.Fragment>
