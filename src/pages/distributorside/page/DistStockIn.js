  ...
  //  const navigate = useNavigate();
   ...
   useEffect(() => {
    const fetchStockData = async () => {
      ...
      try {
        // API call with pagination, search term, and username
        const response = await callApi({
          url: `auth/allstockin/`,
          method: "GET",
          params: {
            distributor_name: username, // Pass the username correctly
          },
        });
  
        console.log("Fetched Data:", response);
        setStockData(response.data.verified_bags || response.data); // Update stock data
   ...
   const rows = stockData.map((item, index) => ({
     ...
     stock_status: item.DistributorStock_status,
   ...
   return (
     <Box component={"div"} sx={{ width: "100%" }}>
       ...
       <Grid container spacing={2} sx={{ marginTop: 5 }}>
         <Grid item xs={12}>
         <Typography variant="h5" mb={2}>DISTRIBUTOR STOCK IN</Typography>
